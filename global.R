...
df <- WDI(country="all", indicator=c("NY.GDP.PCAP.CD", "SP.POP.TOTL", "ST.INT.XPND.CD"), start=2000, end=2015, extra = TRUE)
# Select features to remove
drops <- c("iso2c","iso3c", "capital", "longitude", "latitude", "income", "lending")
df <- df[ , !(names(df) %in% drops)]
# Name features
colnames(df) <- c("country","year", "GDP_per_capita", "population_total", "tourism_expenditure", "region")
...
dataA <- filter(df, region == "Aggregates")
...
colnames(df.Asean) <- c("CountryName","CountryCode","Year","Revenue")
# ===================== csv data: WORLD ===================== 
df.World <- read.csv("Top10Outbound.csv")
colnames(df.World) <- c("CountryName","CountryCode","2005","2006","2007","2008","2009","2010","2011","2012","2013","2014","2015","2016")
df.World <- melt(df.World, id=c(1,2))
colnames(df.World) <- c("CountryName","CountryCode","Year","Revenue")
